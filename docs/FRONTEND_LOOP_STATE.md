...
| 백엔드 개발자 | 494/500 | pykrx KOSPI/KOSDAQ stock universe, KRX 업종 taxonomy, Naver 테마 taxonomy, AI status/RAG fallback structured contract, trade-zone 근거, 이벤트 source-specific causal score와 뉴스 검색/기사 본문/DART 텍스트 signal contract 연결 | live RAG 검증 부족, causal score는 아직 LLM/NLP 판정이 아닌 휴리스틱 |
...
| VC/투자자 | 469/500 | AI/RAG 구조, chart-first/search-first 방향, 전체 종목/KRX 업종/Naver 테마 검색, LLM 설정 가시성, trade-zone 근거, 차트 이벤트 tooltip/source/causal score/news article body signal, AI 답변 구조화, 프론트 상태 경계 개선 | 실제 LLM/RAG moat와 product polish 증거 부족 |

## 11. 다음 루프 계획

1. 이번 event article-body causal scoring 루프 변경분을 의미 있는 단위로 commit/push한다.
2. live LLM/RAG 검증을 우선하되, secret이 없으면 DART 상세 공시 페이지 파싱과 더 정교한 원인 NLP scoring을 진행한다.
