...
- [x] Event text causal scoring 루프 `python3 -m py_compile marketdata-python/app/main.py`: 통과
- [x] Event text causal scoring 루프 `./gradlew test --tests com.krbrief.stocks.StockControllerTest`: 통과
- [x] Event text causal scoring 루프 `npm run build`: 통과
- [x] Event text causal scoring 루프 Docker 기준 `GET /api/stocks/005930/events`: news causal score에 `signalCount=5`, `matchedSignals=거래량/계약/공급/공시/반도체/상승`, `signalSummary` 반환
- [x] Event text causal scoring 루프 최종 `make quality`: ops-check, backend test, frontend build/audit, Docker rebuild/health, investment scan, API smoke, Playwright `13 passed`
- [x] Event text causal scoring 루프 스크린샷: `/tmp/krbrief-screens/event-text-causal-score-1440.png`
- [x] Event article-body causal scoring 루프 `python3 -m py_compile marketdata-python/app/main.py`: 통과
- [x] Event article-body causal scoring 루프 `./gradlew test --tests com.krbrief.stocks.StockControllerTest`: 통과
- [x] Event article-body causal scoring 루프 `npm run build`: 통과
- [x] Event article-body causal scoring 루프 Docker 기준 `GET /api/stocks/005930/events`: `signalOrigins=["article_body","search_result"]`, `signalUrls` 3개, article body 기반 signal 27개 확인
- [x] Event article-body causal scoring 루프 최종 `make quality`: ops-check, backend test, frontend build/audit, Docker rebuild/health, investment scan, API smoke, Playwright `13 passed`
- [x] Event article-body causal scoring 루프 스크린샷: `/tmp/krbrief-screens/event-article-body-causal-score-1440.png`
- [x] DART filing-detail causal scoring 루프 `python3 -m py_compile marketdata-python/app/main.py`: 통과
- [x] DART filing-detail causal scoring 루프 `./gradlew test --tests com.krbrief.stocks.StockControllerTest`: 통과
- [x] DART filing-detail causal scoring 루프 Docker 기준 `GET /api/stocks/005930/events?from=2025-11-01&to=2026-05-04`: `dart_filing_detail` origin 27개, `signalUrls` 실제 `rcpNo` 상세 URL 3개, 공시 본문 `signalSummary` 확인
- [x] DART filing-detail causal scoring 루프 최종 `make quality`: ops-check, backend test, frontend build/audit, Docker rebuild/health, investment scan, API smoke, Playwright `13 passed`
- [x] Playwright 품질 게이트 안정화: npm 캐시 재설치 후 `@playwright/test`를 `1.59.1` exact pinning해 `playwright/lib/program.js` 누락 재발 위험 축소
- [x] Causal factor scoring 루프 `python3 -m py_compile marketdata-python/app/main.py`: 통과
- [x] Causal factor scoring 루프 `./gradlew test --tests com.krbrief.stocks.StockControllerTest`: 통과
- [x] Causal factor scoring 루프 `npm ci --include=dev && npm run build`: 통과
- [x] Causal factor scoring 루프 Docker 기준 `GET /api/stocks/005930/events?from=2025-11-01&to=2026-05-04`: `causalFactors`, `causalDirection`, `evidenceLevel` 반환 확인. 뉴스 sample은 `evidenceLevel=body`, DART sample은 `sourceType=disclosure`, `score=62`, `confidence=medium`, `causalFactors=["투자/증설","자본조달/지분 변동","업종/테마 모멘텀","수주/공급 계약","주주환원"]`
- [x] Causal factor scoring 루프 `./scripts/test_all_apis.sh`: `causalFactors/causalDirection/evidenceLevel` 필드 포함 ALL PASS
...
- [x] Repeatable LLM benchmark 루프 `python3 -m py_compile ai-service/app/main.py scripts/benchmark_llm_quality.py`: 통과
- [x] Repeatable LLM benchmark 루프 `docker compose up -d --build ai-service backend && make health`: 통과
