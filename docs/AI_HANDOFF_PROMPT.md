...
- [x] 첫 화면을 `오늘의 시장 브리프` 중심으로 재배치
- [x] 운영 버튼(생성/백필/보관)을 접힌 운영 관리 패널로 분리
- [x] TOP3 종목 클릭 후 종목 상세 리서치 패널로 연결
...
Phase 1. 프론트 제품 전환
- [x] 첫 화면을 달력 중심에서 `오늘의 시장 브리프` 중심으로 재구성
- [x] 사용자가 앱을 열자마자 오늘 시장, 주요 종목, 차트형 신호, AI 질문창이 보이게 만들기
- [x] 운영 버튼(생성/백필/보관)은 관리자 영역 또는 접힌 패널로 분리
- [ ] 초보자 문구를 더 부드럽고 직관적으로 다듬기
- [x] TOP3 종목 카드를 클릭하면 종목 상세 화면으로 이동하는 구조 만들기
- [ ] 라우팅 구조 도입 검토
- [ ] 모바일/태블릿/데스크톱 반응형 완성
