...
| VC / shareholder | 468/500 | AI and chart-first/search-first platform direction plus all-stock, KRX industry, Naver theme search, LLM configuration visibility, trade-zone evidence, event evidence sources, source-specific causal scores, news text signals, structured AI answers, and frontend state boundaries are clearer, but live LLM/RAG moat is not fully proven |
...
| Event causal scoring | `GET /api/stocks/{code}/events` now returns source-specific `causalScores` with score, confidence, basis, interpretation, signal count, matched keywords, and signal summary; event list and chart tooltip render the top causal score and available text evidence | Done |
...
5. Trade zones now use support/resistance, moving average, and volume-strength evidence; event causal scoring now uses news search text signals, but still needs deeper article body and DART filing detail parsing.
...
2. If live credentials are not available, parse article body pages and DART filing details for stronger causal scoring instead of search-result text only.
