...
- **미래 날짜는 생성할 수 없다.** (400 `future_date_not_allowed` 반환)
...
- **미래 날짜는 보관할 수 없다.** (400 `future_date_not_allowed` 반환)
...
- **미래 날짜가 포함된 백필은 불가능하다.** (`from` 또는 `to`가 오늘 이후면 400 `future_date_not_allowed` 반환)
