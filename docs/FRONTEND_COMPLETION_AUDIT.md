...
| Chart is central and non-empty | `StockPriceChart.jsx` uses `lightweight-charts` for candlestick, MA20, volume, event markers, and restrained price lines; E2E verifies chart canvas on Home and Chart. | Verified |
...
| Keyboard accessibility | Skip link moves keyboard users directly to `#main-content`; selected segmented controls expose `aria-pressed`; focus-visible styling is defined globally. | Verified |
...
- `npm run test:e2e -- --reporter=line`: 9 passed.
