...
| Chart labels stay readable | Default series last-value labels are hidden in favor of one current-price guide, and event markers suppress text labels so the event list carries detail without crowding the axis. | Verified by screenshot |
...
| Keyboard/accessibility | Skip link moves keyboard users directly to `#main-content`; selected segmented controls expose `aria-pressed`; the chart surface has an accessible image role and label; AI/admin/date controls have explicit labels; focus-visible styling is defined globally. | Verified |
